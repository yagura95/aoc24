...
fn main() {
    let mut file = File::open("./day8-1.txt").unwrap();
    ...
    antennas.iter().for_each(|antenna| {
        antenna.1.iter().for_each(|tower| {
            antenna.1.iter().for_each(|t| {
                ...
                if col != 0 && line != 0 {
                    if !anti_nodes.contains(tower) {
                        anti_nodes.push(tower.clone());
                    }
                    while q > -1 && q < grid_len.0 && w > -1 && w < grid_len.1 {
                        let node = (q , w);
    ...
    grid.iter().for_each(|l| println!("{:?}", l.join("")));

    println!("Result: {:?}", anti_nodes.len());
}
